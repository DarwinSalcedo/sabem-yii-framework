...
class EspirituBomberilController extends Controller
{
	...
	public function actionCreate1($id)
	{
		...
		$funcionario = Postulados::model()->findByPk($id);
		$evaluador = Evaluador::model()->findByAttributes(array('CedEvaluador'=>$funcionario->cedula_fun_recepcion,'id_conf_asc_fecha'=>$funcionario->id_conf_asc_fecha));
...
//		    	$fecha = $_POST['EspirituBomberil']['id_conf_asc_fecha']);
			...
			$model->id_evaluador = $evaluador->id_evaluador;
			$model->id_conf_asc_fecha = $funcionario->id_conf_asc_fecha;
			if($model->save()){
				$alerta= new alerta;
				$alerta->generar('EspirituBomberil',$model->Cedula);
				$this->redirect(array('postulados/view','id'=>$funcionario->id_postulados));
			}
		}
