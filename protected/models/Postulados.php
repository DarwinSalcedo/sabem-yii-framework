...
class Postulados extends CActiveRecord
{
	...
	{		
       ...
       # $criteria->addCondition('cedula_fun_recepcion=:ced'); 
        ...
        if($model->getConfAscFecha())
            $criteria->params=array(':fecha_asc'=>$model->getConfAscFecha()->id_conf_asc_fecha);
           # $criteria->params=array(':ced'=>yii::app()->user->getState('cedula'),':fecha_asc'=>$model->getConfAscFecha()->id_conf_asc_fecha);
        else
            $criteria->params=array(':fecha_asc'=>0);
	...
	{		
		$model = new Postulados;
		if($model->getConfAscFecha()){
		$fecha=$model->getConfAscFecha()->id_conf_asc_fecha;		
		}else{
			$fecha=0;
		}
	...
	public function listFuncionarios($cedula)
	{		
		$criteria = new CDbCriteria;                                      
        $criteria->addCondition('NOT Cedula=:ced');  
        $criteria->params=array(':ced'=>$cedula);                    
    	return CHtml::listData(Funcionarios::model()->findAll($criteria),'Cedula','NombreCedula') ;
	}
